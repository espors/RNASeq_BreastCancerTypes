...
counts <- counts[which(apply(cpm(DGEList(counts = counts)), 1, 
                             function(y) sum(y >= 0.5)) >= 3), ]
...
hist(
  assay(dds), 
  main = "Histogram of Original Data", 
  breaks = 20
)
hist(
  assay(transformed), 
  main = "Histogram of Data with VTS Transformation"
)
```
...
```{r}
select <- order(apply(assay(transformed), 1, sd), decreasing = T)[1:150]

df <- as.data.frame(colData(dds))[, c("type")]
df

pheatmap::pheatmap(
  mat = assay(transformed)[select, ], 
  cluster_rows = FALSE, 
  show_rownames = FALSE, 
  cluster_cols = FALSE,
  #annotation_col = df, 
...
pheatmap::pheatmap(
  mat = assay(transformed)[select, ] - rowMeans(assay(transformed)[select, ]), 
  cluster_rows = FALSE, 
  show_rownames = FALSE, 
  cluster_cols = FALSE, 
  #annotation_col = df, 
...
ggplot(
  pcaData, 
  aes(PC1, PC2, color = type, shape = type)
) + 
  geom_point(size = 3) + 
...
plotCounts(dds, gene = topGene, intgroup = ("type")) 
